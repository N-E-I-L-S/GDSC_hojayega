...
const Bar = () => {
  ...
  async function getData() {
    const res = await fetch('http://127.0.0.1:5000/top-item-quantity');
    const json = await res.json();
    const formattedData = json.map(item => ({ x: item.item_name, y: item.quantity}));
  ...
  if (barData.length === 0)
    ...
    return (
      <div className="m-4 md:m-10 mt-24 p-10 bg-white dark:bg-secondary-dark-bg rounded-3xl">
        <ChartsHeader title={"Item Sales"} description={description} /> 
